...
export class GammaSDK {
  ...
  // Keyset Pagination API (cursor-based, more efficient than offset for large datasets)

  /**
   * Get events with cursor-based (keyset) pagination
   *
   * @param query - Query parameters including optional after_cursor for pagination
   * @returns Promise resolving to events array with optional next_cursor
   *
   * @example
   * ```ts
   * const page1 = await gamma.getEventsKeyset({ limit: 20, active: true });
   * if (page1.next_cursor) {
   *   const page2 = await gamma.getEventsKeyset({ limit: 20, after_cursor: page1.next_cursor });
   * }
   * ```
   */
  async getEventsKeyset(
    query: Record<string, any> = {},
  ): Promise<{ data: EventType[]; next_cursor?: string }> {
    const response = await this.makeRequest<{
      data: any[];
      next_cursor?: string;
    }>("/events", { ...query, pagination_type: "keyset" });
    const raw = this.extractResponseData(response, "Get events keyset");

    // Handle both array response (no cursor) and paginated object response
    if (Array.isArray(raw)) {
      return {
        data: (raw as any[]).map((item) => this.transformEventData(item)),
      };
    }

    return {
      data: (raw.data ?? []).map((item: any) => this.transformEventData(item)),
      next_cursor: raw.next_cursor,
    };
  }

  /**
   * Get markets with cursor-based (keyset) pagination
   *
   * @param query - Query parameters including optional after_cursor for pagination
   * @returns Promise resolving to markets array with optional next_cursor
   *
   * @example
   * ```ts
   * const page1 = await gamma.getMarketsKeyset({ limit: 20, active: true });
   * if (page1.next_cursor) {
   *   const page2 = await gamma.getMarketsKeyset({ limit: 20, after_cursor: page1.next_cursor });
   * }
   * ```
   */
  async getMarketsKeyset(
    query: Record<string, any> = {},
  ): Promise<{ data: MarketType[]; next_cursor?: string }> {
    const response = await this.makeRequest<{
      data: any[];
      next_cursor?: string;
    }>("/markets", { ...query, pagination_type: "keyset" });
    const raw = this.extractResponseData(response, "Get markets keyset");

    // Handle both array response (no cursor) and paginated object response
    if (Array.isArray(raw)) {
      return {
        data: (raw as any[]).map((item) => this.transformMarketData(item)),
      };
    }

    return {
      data: (raw.data ?? []).map((item: any) => this.transformMarketData(item)),
      next_cursor: raw.next_cursor,
    };
  }
}
