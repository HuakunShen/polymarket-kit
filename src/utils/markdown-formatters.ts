 ...
 */
export function formatMarketToMarkdown(market: MarketData, options: MarkdownOptions = {}): string {
	...
	if (verbose === 0) {
		if (market.startDateIso || market.endDateIso) {
			parts.push(`**Trading Period**: ${market.startDateIso || 'N/A'} → ${market.endDateIso || 'N/A'}`);
		}
		return parts.join('\n\n') + '\n\n---\n';
	}
	...
	parts.push(`**Status**: ${market.active ? 'Active' : 'Inactive'} | ${market.closed ? 'Closed' : 'Open'} | ${market.restricted ? 'Restricted' : 'Unrestricted'}`);
	...
	if (verbose === 1) {
		const basicMetrics = [];
		if (market.volumeNum !== undefined) basicMetrics.push(`Volume: $${market.volumeNum.toLocaleString()}`);
		else if (market.volume !== undefined) basicMetrics.push(`Volume: $${market.volume}`);
	...
	if (market.lastTradePrice !== undefined) tradingMetrics.push(`Last Trade: $${market.lastTradePrice}`);
	if (market.bestBid !== undefined) tradingMetrics.push(`Best Bid: $${market.bestBid}`);
	if (market.bestAsk !== undefined) tradingMetrics.push(`Best Ask: $${market.bestAsk}`);
	if (market.spread !== undefined) tradingMetrics.push(`Spread: ${(market.spread * 100).toFixed(2)}%`);
	...
	const priceChanges = [];
	if (market.oneHourPriceChange !== undefined) priceChanges.push(`1hr: ${(market.oneHourPriceChange * 100).toFixed(2)}%`);
	if (market.oneDayPriceChange !== undefined) priceChanges.push(`24hr: ${(market.oneDayPriceChange * 100).toFixed(2)}%`);
	...
	const volumeMetrics = [];
	if (market.volumeNum !== undefined) volumeMetrics.push(`Total: $${market.volumeNum.toLocaleString()}`);
	else if (market.volume !== undefined) volumeMetrics.push(`Total: $${market.volume}`);
	if (market.volume24hr !== undefined) volumeMetrics.push(`24hr: $${market.volume24hr.toLocaleString()}`);
	if (market.volume1wk !== undefined) volumeMetrics.push(`1wk: $${market.volume1wk.toLocaleString()}`);
	if (market.liquidityNum !== undefined) volumeMetrics.push(`Liquidity: $${market.liquidityNum.toLocaleString()}`);
	...
	if (market.enableOrderBook) {
		const constraints = [];
		if (market.orderMinSize !== undefined) constraints.push(`Min Size: $${market.orderMinSize}`);
		if (market.orderPriceMinTickSize !== undefined) constraints.push(`Min Tick: $${market.orderPriceMinTickSize}`);
	...
	if (market.description) {
		const shortDesc = market.description.length > 200
...
function formatSeriesToMarkdown(series: SeriesData[]): string {
	...
	series.forEach(s => {
		const metrics = [];
		if (s.volume !== undefined) metrics.push(`Volume: $${s.volume.toLocaleString()}`);
		if (s.liquidity !== undefined) metrics.push(`Liquidity: $${s.liquidity.toLocaleString()}`);
		if (s.commentCount !== undefined) metrics.push(`Comments: ${s.commentCount}`);

		parts.push(`**${s.title}** (${s.seriesType}, ${s.recurrence}) - ${s.active ? 'Active' : 'Inactive'} | ${metrics.join(' | ')}`);
	});
 ...
 */
export function formatEventToMarkdown(event: EventData, options: MarkdownOptions = {}): string {
	...
	parts.push(`**Status**: ${event.active ? 'Active' : 'Inactive'} | ${event.closed ? 'Closed' : 'Open'} | ${event.archived ? 'Archived' : 'Live'} | ${event.restricted ? 'Restricted' : 'Unrestricted'}`);
	...
	if (verbose === 1) {
		...
		if (event.commentCount !== undefined) basicMetrics.push(`Comments: ${event.commentCount}`);
		parts.push(`**Metrics**: ${basicMetrics.join(' | ')}`);

		// Event description
		if (event.description) {
			const shortDesc = event.description.length > 250
	...
	if (event.openInterest !== undefined) eventMetrics.push(`Open Interest: $${event.openInterest.toLocaleString()}`);
	if (event.volume24hr !== undefined) eventMetrics.push(`24hr Vol: $${event.volume24hr.toLocaleString()}`);
	if (event.volume1wk !== undefined) eventMetrics.push(`1wk Vol: $${event.volume1wk.toLocaleString()}`);
	if (event.commentCount !== undefined) eventMetrics.push(`Comments: ${event.commentCount}`);

	parts.push(`**Event Metrics**: ${eventMetrics.join(' | ')}`);

	// Order book availability
	if (event.enableOrderBook !== undefined) {
		parts.push(`**Order Book**: ${event.enableOrderBook ? 'Enabled' : 'Disabled'}`);
	}
	...
	if (includeMarkets) {
		...
		if (event.markets.length > 1) {
			analysisPoints.push('- **Cross-Market Opportunities**: Multiple markets may allow for arbitrage strategies');
		}

		const activeMarkets = event.markets.filter(m => m.active && !m.closed);
		if (activeMarkets.length > 0) {
			const avgSpread = activeMarkets
				.filter(m => m.spread !== undefined)
				.reduce((sum, m) => sum + (m.spread || 0), 0) / activeMarkets.length;
			if (avgSpread > 0) {
				analysisPoints.push(`- **Average Spread**: ${(avgSpread * 100).toFixed(2)}% across active markets`);
			}
		}
