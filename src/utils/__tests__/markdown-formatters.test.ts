...
import { formatEventToMarkdown, formatMarketToMarkdown, type EventData, type MarketData } from "../markdown-formatters";

// Sample market data for testing
const sampleMarket: MarketData = {
	id: "581554",
	question: "1,415+ Measles cases in U.S. by September 12?",
	conditionId: "0xb5454a5643e2f61fa78d7cd1039049f4208f9fc995c3eee19c164d58e9ff3fdc",
	slug: "1415-measles-cases-in-us-by-september-12",
	description: "This market will resolve to \"Yes\" if there have been 1,415 or more confirmed cases of Measles (Rubeola) in humans in the territory of the United States of America in 2025, according to the CDC case counter, by September 12, 2025, 11:59 PM ET. Otherwise, this market will resolve to \"No\".",
	outcomes: ["Yes", "No"],
	outcomePrices: ["1", "0"],
	volume: "9916.517481",
	active: true,
	...
	volume24hr: 8987.906230,
	volume1wk: 8987.906230,
...
describe("formatMarketToMarkdown", () => {
	test("should format market with default verbose level (2)", () => {
		...
		expect(result).toContain("## Market: 1,415+ Measles cases in U.S. by September 12?");
...
describe("formatEventToMarkdown", () => {
	test("should format event with default options (verbose=2, includeMarkets=true)", () => {
		...
		expect(result).toContain("# Event: 1,415+ Measles cases in U.S. by September 12?");
		expect(result).toContain("**ID**: 39822 | **Slug**: 1415-measles-cases-in-us-by-september-12");
		expect(result).toContain("**Status**: Active | Closed | Live | Restricted");
		expect(result).toContain("**Timeline**: 2025-08-26T16:47:21.777321Z → 2025-09-12T00:00:00Z");
	...
	test("should format event without markets when includeMarkets=false", () => {
		const result = formatEventToMarkdown(sampleEvent, { includeMarkets: false });
		...
		expect(result).not.toContain("## Market: 1,415+ Measles cases in U.S. by September 12?");
		expect(result).not.toContain("## Arbitrage Analysis Summary");
	});
	...
	test("should handle event with multiple markets", () => {
		const eventWithMultipleMarkets: EventData = {
			...sampleEvent,
			markets: [sampleMarket, { ...sampleMarket, id: "123456", question: "Another question?" }],
		};

		const result = formatEventToMarkdown(eventWithMultipleMarkets);
		expect(result).toContain("## Markets (2 total)");
		expect(result).toContain("**Cross-Market Opportunities**: Multiple markets may allow for arbitrage strategies");
	});
});
...
describe("Real-world usage examples", () => {
	test("should demonstrate how users can map events to markdown", () => {
		...
		expect(basicMarkdowns[0]).toContain("# Event: 1,415+ Measles cases in U.S. by September 12?");
	...
	test("should demonstrate filtering events with markets for arbitrage analysis", () => {
		...
		const arbitrageOpportunities = events
			...
			.map(event => formatEventToMarkdown(event, { verbose: 2, includeMarkets: true }));
		...
		const multiMarketEvent: EventData = {
			...sampleEvent,
			markets: [sampleMarket, { ...sampleMarket, id: "999", question: "Related question?" }]
		};
	...
	test("should demonstrate batch processing for different analysis needs", () => {
		...
		if (screening.length === 0 || overview.length === 0 || fullAnalysis.length === 0) {
