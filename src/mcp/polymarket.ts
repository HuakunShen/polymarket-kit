...
function formatMarketDisplay(markets: MarketType[]): string {
  ...
  const marketList = markets.map((market, index) => {
    ...
    const volumeInfo = market.volumeNum ? `$${market.volumeNum.toLocaleString()}` : market.volume;
    const outcomeInfo = market.outcomes && market.outcomePrices ? 
      market.outcomes.map((outcome, i) => `${outcome}: $${market.outcomePrices![i]}`).join(", ") : "";
...
function formatSearchDisplay(results: SearchResponseType): string {
  ...
  if (results.events && results.events.length > 0) {
    ...
    results.events.forEach((event: any, index) => {
      sections.push(`${index + 1}. ${event.title || event.name || "Unnamed Event"} (ID: ${event.id})`);
    });
    sections.push("");
  }
  ...
  if (results.profiles && results.profiles.length > 0) {
    ...
    results.profiles.forEach((profile: any, index) => {
      sections.push(`${index + 1}. ${profile.name || profile.username || "Unnamed Profile"}`); 
    });
    sections.push("");
  }
  ...
  if (results.tags && results.tags.length > 0) {
    ...
    results.tags.forEach((tag: any, index) => {
      sections.push(`${index + 1}. ${tag.label || tag.name || "Unnamed Tag"} (${tag.slug || "no-slug"})`);
    });
  }
...
server.tool(
  ...
  {
    limit: z.number().min(1).max(100).optional().describe("Maximum number of markets to return (1-100)"),
...
server.tool(
  ...
  async (params) => {
    try {
      ...
      if (params.include_tag !== undefined) query.include_tag = params.include_tag;
...
server.tool(
  ...
  {
    limit: z.number().min(1).max(100).optional().describe("Maximum number of events to return"),
...
server.tool(
  ...
  async (params) => {
    try {
      ...
      if (params.include_chat !== undefined) query.include_chat = params.include_chat;
...
server.tool(
  ...
  "Convert event data to markdown format optimized for AI analysis",
  {
    ...
    verbose: z.union([z.literal(0), z.literal(1), z.literal(2)]).optional().describe("Detail level: 0=basic, 1=medium, 2=full"),
...
server.tool(
  ...
  "Perform comprehensive search across markets, events, and profiles",
  {
    ...
    limit_per_type: z.number().min(1).max(50).optional().describe("Results per category"),
  ...
  async (params) => {
    try {
      ...
      if (params.limit_per_type !== undefined) query.limit_per_type = params.limit_per_type;
      if (params.events_status !== undefined) query.events_status = params.events_status;
...
server.tool(
  ...
  {
    limit: z.number().min(1).max(100).optional().describe("Maximum tags to return"),
  ...
  async (params) => {
    try {
      ...
      if (params.is_carousel !== undefined) query.is_carousel = params.is_carousel;
    ...
    try {
      ...
      const trendAnalysis = [
        ...
        ...(trendingMarkets.map((market, index) => {
          const volume = market.volumeNum ? `$${market.volumeNum.toLocaleString()}` : "N/A";
...
server.tool(
  ...
  {
    period: z.enum(["24h", "7d", "30d"]).optional().describe("Time period for popularity calculation"),
...
async function main() {
  ...
  console.error("Available tools: get_markets, get_market_by_id, get_market_by_slug, get_events, get_event_by_id, get_event_markdown, search_polymarket, get_tags, get_market_trends, get_popular_markets");
