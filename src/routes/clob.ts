...
async function getPolymarketSDK(
  ...
  // Update proxy if provided (for both new and cached instances)
  if (proxyUrl) {
    // console.log("Setting proxy URL for polymarket SDK:", proxyUrl);
...
export const clobRoutes = new Elysia({ prefix: "/clob" })
  ...
  .resolve(async ({ headers }) => {
    ...
    const polymarketSDK = await getPolymarketSDK(privateKey, funderAddress, proxyHeaderValue);
